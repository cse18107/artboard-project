...
import { useDeleteComment, useDeleteThread, useMarkThreadAsResolved } from "@liveblocks/react";
...
export const PinnedThread = ({ thread, onFocus, ...props }: Props) => {
  ...
  const deleteThread = useMarkThreadAsResolved();
        ...
        {!minimized ? (
          <div className='flex min-w-60 flex-col overflow-hidden rounded-lg bg-white text-sm shadow'>
            <Thread
              ...
              onResolvedChange={(value) => deleteThread(thread.id)}
            />
          </div>
        ) : null}
      </div>
